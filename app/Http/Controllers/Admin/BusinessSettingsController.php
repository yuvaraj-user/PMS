...
class BusinessSettingsController extends Controller
{
    ...
    {
        $search = $request->search;
        $wishlist = wishlist::with(['customer','product_full_info'])->whereHas('customer',function($query) use($search) {
            if($search) {
                $query->where('f_name','Like','%'.$search.'%');
            }
        })->orWhereHas('product_full_info',function($qry) use($search) {
            $qry->where('name','Like','%'.$search.'%');
        })->paginate(Helpers::pagination_limit());
        
        return view('admin-views.business-settings.wishlist', compact('wishlist','search'));
    }

    public function wishlist_export(Request $request)
    {
        $search = $request->excel_search;
        $wishlist = wishlist::with(['customer','product_full_info'])->whereHas('customer',function($query) use($search) {
            if($search) {
                $query->where('f_name','Like','%'.$search.'%');
            }
            ...
            return  [
                'sno'           => $key+1,
                'customer_name' => $data->customer->f_name,
                'product_name'  => $data->product_full_info->name,
                'created_at'    => date('d-m-y H:i:s A',strtotime($data->created_at)),
                'updated_at'    => date('d-m-y H:i:s A',strtotime($data->update_at))
            ];
        });
        return (new FastExcel($wishlist))->download('wishlist.xlsx');
    }

    public function wishlist_push_notification(Request $request)
    {
        $customer = wishlist::with(['customer'])->groupBy('customer_id')->get();
        $product  = wishlist::with(['customer','product_full_info'])->get();
        return view('admin-views.business-settings.wishlist_push_notification',compact('customer','product'));
    }

    public function wishlist_send_notification(Request $request)
    {
        $request->validate([
            'title' => 'required',
