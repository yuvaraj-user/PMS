    ...
    public function variant_price(Request $request)
    {
        ...
        if ($str != null) {
            ...
            if(\Auth::guard('customer')->check() && \Auth::guard('customer')->user()->is_dealer == 1) {
                $tax = $product->tax_model=='exclude' ? Helpers::tax_calculation($product->dealer_unit_price, $product['tax'], $product['tax_type']) : 0;
                $discount = Helpers::get_product_discount($product, $product->dealer_unit_price);
                $price = $product->dealer_unit_price - $discount + $tax;
